...
// Create a Proxy that allows adding new properties while delegating to original THREE
const THREE_EXTENSIONS = {};
const THREE = new Proxy(THREE_ORIGINAL, {
	get(target, prop) {
		if (prop in THREE_EXTENSIONS) return THREE_EXTENSIONS[prop];
		return target[prop];
	},
	set(target, prop, value) {
		THREE_EXTENSIONS[prop] = value;
		return true;
	},
	has(target, prop) {
		return prop in THREE_EXTENSIONS || prop in target;
	}
});
...
Object.assign(THREE.TGXLoader.prototype, {
	load: function (options, onLoad, onProgress, onError) {
		...
		function gearAsset(/*itemIndex*/itemHash, callback) {
			...
			loader.load(url, options.apiKey, function (response) {
				...
				console.log('[TGXLoader] API Response for', itemHash, ':', response);

				if (response.ErrorCode == 1) {
					// Destiny 2 API may have different structure than D1
					var data = response.Response;
					if (data && data.data) {
						data = data.data; // D1 style
					}
					if (data) {
						callback(data);
					} else {
						console.error('[TGXLoader] No data in response:', response);
					}
				} else {
	...
	parse: function (items, shaders, options, onLoad, onProgress, onError) {
		...
		function loadAssetManifest(gear) {
			...
			// Load Bones / Animations (always from destiny_content, shared between D1 and D2)
			if (loadSkeleton && contentLoaded.skeleton == undefined) {
				contentLoaded.skeleton = null;
				var animContentPath = basepath + '/common/destiny_content';
				loadPart(animContentPath + '/animations/destiny_player_skeleton.js', function (skeleton) {
				...
				if (loadAnimation) {
					loadPart(animContentPath + '/animations/' + animationPath, function (animations) {
		...
		function checkContentLoaded() {
			console.log('[TGXLoader] checkContentLoaded:', assetLoadCount, '/', assetLoadTotal);
			if (assetLoadCount < assetLoadTotal) return;
			if (contentParsed) return;
			contentParsed = true;

			console.log('[TGXLoader] All assets loaded, calling parseContent');
		...
		function parseContent() {
			console.log('[parseContent] Starting...');
			console.log('[parseContent] ContentLoaded:', contentLoaded);

			try {
				// Set up THREE.Geometry and load skeleton (if any)
				console.log('[parseContent] Creating THREE.Geometry...');
				geometry = new THREE.Geometry();
				console.log('[parseContent] Geometry created:', geometry);

				console.log('[parseContent] Parsing skeleton...');
				geometry.bones = parseSkeleton();
				hasBones = geometry.bones.length > 0;
				console.log('[parseContent] Skeleton parsed, hasBones:', hasBones);

				var animation = hasBones && loadAnimation ? parseAnimation(geometry.bones) : false;
				console.log('[parseContent] Animation:', animation);
				...
				console.log('[parseContent] Default material created');

				vertexOffset = 0;
				console.log('[parseContent] Parsing', contentLoaded.items.length, 'items...');
				for (var i = 0; i < contentLoaded.items.length; i++) {
					console.log('[parseContent] Parsing item', i, contentLoaded.items[i]);
					parseItem(contentLoaded.items[i]);
				}
				console.log('[parseContent] All items parsed, geometry vertices:', geometry.vertices.length);

				if (typeof onLoadCallback !== 'function') {
					console.warn('[parseContent] NoOnLoadCallback!', geometry, materials, animation);
					return;
				}
				console.log('[parseContent] Calling onLoadCallback...');
				onLoadCallback(geometry, materials, animation ? [animation] : []);
				console.log('[parseContent] Complete!');
			} catch (error) {
				console.error('[parseContent] ERROR:', error);
				console.error('[parseContent] Stack:', error.stack);
			}
		}
		...
		function parseItem(item) {
			...
			console.log('ParseItem gear:', gear);

			// Handle case where gear is not in contentLoaded.gear (D2 mobile manifest)
			if (!gear) {
				console.warn('[parseItem] gear not found for', item.requestedId, '- using gearAsset data directly');
				console.log('[parseItem] item.gearAsset:', JSON.stringify(item.gearAsset, null, 2));
				console.log('[parseItem] item.gearAsset.content[0]:', item.gearAsset.content[0]);
			...
			if (artContent) {
				...
				// D2 Mobile Manifest fallback - no art_content available
				console.log('[parseItem] D2 Mobile mode - checking for index sets');
				var content0 = item.gearAsset.content[0];
				console.log('[parseItem] content0:', content0);

				if (content0) {
					// First try region_index_sets (some D2 items)
					if (content0.region_index_sets) {
						console.log('[parseItem] Using region_index_sets');
						for (var regionIdx in content0.region_index_sets) {
							var regionSet = content0.region_index_sets[regionIdx];
							if (regionSet && regionSet.length > 0) {
								var setEntry = regionSet[0];
								if (setEntry && setEntry.geometry && setEntry.geometry.length > 0) {
									var geometryHashes = setEntry.geometry.map(function (geoIdx) {
										return content0.geometry[geoIdx];
									});
									artRegionPatterns.push({
										hash: 0,
										artRegion: parseInt(regionIdx),
										patternIndex: 0,
										regionIndex: parseInt(regionIdx),
										geometry: geometryHashes
									});
									console.log('[parseItem] Added region', regionIdx, ':', geometryHashes);
								}
							}
						}
					}

					// D2 armor uses male_index_set / female_index_set instead
					var genderIndexSet = isFemale ? content0.female_index_set : content0.male_index_set;
					if (genderIndexSet && genderIndexSet.geometry && genderIndexSet.geometry.length > 0) {
						console.log('[parseItem] Using gender index set:', isFemale ? 'female' : 'male', genderIndexSet);
						var geometryHashes = genderIndexSet.geometry.map(function (geoIdx) {
							return content0.geometry[geoIdx];
						});
						artRegionPatterns.push({
							hash: 0,
						...
						console.log('[parseItem] Added gender geometry:', geometryHashes);
					}
				}

				console.log('[parseItem] artRegionPatterns:', artRegionPatterns.length);
			}
			...
			for (var a = 0; a < artRegionPatterns.length; a++) {
				...
				for (var g = 0; g < artRegionPattern.geometry.length; g++) {
					...
					// D2 fallback: try to find in tgxms by filename (without extension)
					if (tgxBin == undefined && geometryHash) {
						var geoName = geometryHash.split('.')[0]; // Remove .tgxm.bin extension
						tgxBin = contentLoaded.tgxms && contentLoaded.tgxms.geometry ? contentLoaded.tgxms.geometry[geoName] : undefined;
						if (tgxBin) {
							console.log('[parseItem] Found geometry in tgxms:', geoName);
							// Store in contentLoaded.geometry so parseTextures and parseGeometry can find it
							contentLoaded.geometry[geometryHash] = tgxBin;
						}
					}
					...
					//if (g != 1) continue;
					if (tgxBin == undefined) {
						console.warn('MissingGeometry[' + g + ']', geometryHash, 'Available keys:', Object.keys(contentLoaded.geometry || {}).slice(0, 5));
						continue;
					}
		...
		function parseMaterial(part, gearDye, geometryTextures) {
			...
			if (part.shader) {
				...
				// Also check if primaryColor exists (D2 mobile may not have it)
				if (materialParams.primaryColor &&
		...
		function parseTGXAsset(tgxBin, geometryHash) {
			console.log('[parseTGXAsset] tgxBin:', tgxBin);

			if (!tgxBin) {
				console.warn('[parseTGXAsset] tgxBin is undefined for', geometryHash);
				return [];
			}

			var metadata = tgxBin.metadata; // Arrangement
			if (!metadata || !metadata.render_model) {
				console.warn('[parseTGXAsset] No metadata or render_model for', geometryHash, 'Available keys:', Object.keys(tgxBin || {}));
				return [];
			}
...
(function () {
	...
	// UPDATED to use modern @zip.js/zip.js API
	async function loadDatabase(response, onLoad, onProgress, onError) {
		...
		if (assetDatabases.length > 0) {
			...
			console.log('[TGXManifest] Loading database from:', assetDatabaseUrl);

			try {
				// Use modern @zip.js/zip.js API
				const zipModule = window.zip;
				if (!zipModule) {
					console.error('[TGXManifest] zip.js not found on window');
					if (onError) onError(new Error('zip.js not loaded'));
					return;
				}

				// Fetch the zip file
				const zipResponse = await fetch(assetDatabaseUrl);
				if (!zipResponse.ok) {
					throw new Error('Failed to fetch manifest: ' + zipResponse.status);
				}
				const zipBlob = await zipResponse.blob();

				// Use BlobReader and BlobWriter from modern zip.js
				const zipReader = new zipModule.ZipReader(new zipModule.BlobReader(zipBlob));
				const entries = await zipReader.getEntries();

				if (entries.length === 0) {
					console.error('[TGXManifest] Empty Database');
					await zipReader.close();
					if (onError) onError(new Error('Empty database'));
					return;
				}

				// Get first entry as blob
				const firstEntry = entries[0];
				const dbBlob = await firstEntry.getData(new zipModule.BlobWriter());
				await zipReader.close();

				// Convert blob to Uint8Array
				const arrayBuffer = await dbBlob.arrayBuffer();
				manifestBlob = new Uint8Array(arrayBuffer);

				console.log('[TGXManifest] Database loaded, size:', manifestBlob.length);
				if (onLoad) onLoad();
			} catch (err) {
				console.error('[TGXManifest] Error loading database:', err);
				if (onError) onError(err);
			}
		} else {
			console.error('[TGXManifest] No asset databases in manifest');
			if (onError) onError(new Error('No asset databases'));
		}
	}
...
// ES Module exports
export const TGXLoader = THREE.TGXLoader;
export const TGXLoaderUtils = THREE.TGXLoaderUtils;
export const TGXMaterial = THREE.TGXMaterial;
export const TGXManifest = THREE.TGXManifest;
export const BungieNetLoader = THREE.BungieNetLoader;
export default TGXLoader;
